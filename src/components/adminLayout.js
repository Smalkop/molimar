const MENU_ITEMS = [
  ...
  ['/admin/productos', 'Productos', 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4'],
  ['/admin/galeria', 'Galería', 'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z'],
  ['/admin/venta-directa', 'Venta Directa', 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z'],
  ['/admin/usuarios', 'Usuarios', 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z'],
  ['/admin/configuracion', 'Configuración', 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z'],
  ['/admin/mensajes', 'Mensajes', 'M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z'],
];

// header: 'toggle' -> menú móvil con botón para abrir el sidebar
//         'label'  -> barra móvil con solo el título
//         null     -> sin barra móvil
export function adminLayout({ title, active, content, user, header = 'label' }) {
  const headerHtml = header === 'toggle' ? `
    ...
    <header class="bg-white border-b border-gray-200 px-6 py-4 lg:hidden"><div class="flex items-center justify-between"><span class="font-bold text-gray-900">${title}</span></div></header>` : '';
...
<div class="flex h-screen overflow-hidden">
  <aside class="hidden lg:flex lg:flex-col w-64 bg-white border-r border-gray-200">
    ...
    <nav class="flex-1 py-4 space-y-1 px-3">
      ${MENU_ITEMS.map(([href, label, path]) => `
        <a href="${href}" class="sidebar-link flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-medium ${href === active ? 'text-primary-700 bg-primary-50 border-r-2 border-primary-600' : 'text-gray-700'}">
