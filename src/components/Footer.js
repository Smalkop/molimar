...
export function Footer({ settings }) {
  ...
  const socialIcons = {
    ...
    tiktok: '<path fill="currentColor" d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z"/>',
  };
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          <div class="animate-on-scroll">
            ...
            <p class="text-gray-400 text-sm leading-relaxed">${escapeHtml(settings.company_description || 'Producción y comercialización de harinas y fideos de la más alta calidad.')}</p>
          </div>
          ...
          <div class="animate-on-scroll">
            ...
            <ul class="space-y-3">
              ...
              <li><a href="/sucursales" class="text-gray-400 hover:text-white text-sm transition-colors">Centros de Distribución</a></li>
          ...
          <div class="animate-on-scroll">
            ...
            <ul class="space-y-3 text-sm">
              ${settings.address ? `<li class="flex items-start space-x-3"><svg class="w-5 h-5 text-primary-400 mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg><span>${escapeHtml(settings.address)}</span></li>` : ''}
              ${settings.phone ? `<li class="flex items-center space-x-3"><svg class="w-5 h-5 text-primary-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg><a href="tel:${escapeHtml(settings.phone)}" class="hover:text-white transition-colors">${escapeHtml(settings.phone)}</a></li>` : ''}
              ${settings.email ? `<li class="flex items-center space-x-3"><svg class="w-5 h-5 text-primary-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg><a href="mailto:${escapeHtml(settings.email)}" class="hover:text-white transition-colors">${escapeHtml(settings.email)}</a></li>` : ''}
            </ul>
          </div>
          ...
          <div class="animate-on-scroll">
            ...
            ${settings.schedule ? `<p class="text-xs text-gray-500 mt-6 leading-relaxed">${escapeHtml(settings.schedule)}</p>` : ''}
          </div>
        </div>
      </div>
